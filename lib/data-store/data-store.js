...
function SlackDataStore(opts) {
  var store = this;
  forEach(messageHandlers, function (handler, event) {
 ...
 */
SlackDataStore.prototype.handleRtmMessage = function handleRtmMessage(activeUserId, activeTeamId, messageType, message) {
  var handler = void 0;
  if (messageType === 'message' && !isUndefined(message.subtype)) {
    ...
    handler = handler || this._messageHandlers[makeMessageEventWithSubtype('rtm_client_add_message')];
  } else {
