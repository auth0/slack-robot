...
var CLIENT_RTM_EVENTS = _client.CLIENT_EVENTS.RTM;
...
var Robot = function (_EventEmitter) {
  ...
  function Robot(token) {
     ...
     * @private
     */
    _this._rtm = new _client.RtmClient(token, { dataStore: new _client.MemoryDataStore() });
  ...
  _createClass(Robot, [{
    ...
    value: function start() {
      ...
      this._rtm.on(_client.RTM_EVENTS.MESSAGE, function (message) {
      ...
      this._rtm.on(_client.RTM_EVENTS.REACTION_ADDED, function (message) {
        /**
         ...
         */
        if (message.item.type !== _client.RTM_EVENTS.MESSAGE) {
    ...
    value: function _getQueueEntry(message) {
      switch (message.type) {
        case _client.RTM_EVENTS.REACTION_ADDED:
