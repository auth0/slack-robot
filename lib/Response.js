...
var Response = function (_EventEmitter) {
  ...
  function Response(slackToken, dataStore, request) {
     ...
     * @type {WebClient}
     */
    _this._api = new _webApi.WebClient(slackToken, { maxRequestConcurrency: concurrency });
  ...
  _createClass(Response, [{
    ...
    value: function _send(task, callback) {
      ...
      if (task.target.indexOf(USER_PREFIX) > -1) {
        ...
        return this._api.conversations.open({ users: userId }).then(function (data) {
          if (!data.ok) {
    ...
    value: function _sendTextResponse(id, text, callback) {
      var postMessageArguments = Object.assign({}, DEFAUT_POST_MESSAGE_OPTS, {
        text: text,
        channel: id
      });
    ...
    value: function _sendReactionResponse(reaction, callback) {
      ...
      this._api.reactions.add(opts).then(function (res) {
        callback(null, res);
      }).catch(function (err) {
        return callback(err);
      });
    ...
    value: function _sendFileResponse(id, file, callback) {
      var bodyFormData = {
        ...
        filetype: (0, _util.getFileExtension)(file.filename)
      };
      ...
      var options = {
        ...
        data: _querystring2.default.stringify(bodyFormData)
      };
